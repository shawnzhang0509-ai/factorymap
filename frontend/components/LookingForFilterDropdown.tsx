...
interface LookingForFilterDropdownProps {
  /** null = no filter */
  value: LookingForKey | null;
  onChange: (next: LookingForKey | null) => void;
}

const MENU_WIDTH = 280;
const GAP = 8;

const LookingForFilterDropdown: React.FC<LookingForFilterDropdownProps> = ({ value, onChange }) => {
  const triggerRef = useRef<HTMLButtonElement>(null);
  const [open, setOpen] = useState(false);
  const [draft, setDraft] = useState<LookingForKey | null>(null);
...
export default LookingForFilterDropdown;
